...
object Ch08 extends App {
  ...
  object Attempt1 {
    case class Gen[A](sample: State[RNG, A]) { self =>
      ...
      def listOfN(size: Gen[Int]): Gen[List[A]] = size.flatMap(s => Gen.listOfN(s, self))

      def union(g1: Gen[A], g2: Gen[A]): Gen[A] = Gen.boolean.flatMap {
        case true => g1
        case false => g2
      }
    ...
    object Gen {
      ...
      def choose(start: Int, stopExclusive: Int): Gen[Int] = {
        Gen[Int](State[RNG, Int](r => r.nextInt).map(n => start + n % (stopExclusive - start)))
      }
      ...
      def listOf[A](a: Gen[A]): List[Gen[A]] = ???
  ...
  object Attempt2 {
    ...
    object Gen {
      ...
      def choosePair(from: Int, to: Int): Gen[(Int, Int)] = {

        choose(from, to).flatMap { a =>
          choose(from, to).flatMap(b => unit((a, b)))
        }

        // for the following to work we need the withFilter to be present
        //        for {
        //          a: Int <- Gen.choose(from, to)
        //          b: Int <- Gen.choose(from, to)
        //        } yield (a, b)
      ...
      def weighted[A](g1: (Gen[A], Double), g2: (Gen[A], Double)): Gen[A] = {
        ...
        Gen(State(RNG.double).flatMap { d => if (d > weight) g1._1.sample else g2._1.sample })
      }
...
object TestStream {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def go(s: Stream[Int]): Stream[Int] = {
      Stream.unfold[Int, Stream[Int]](s)((l: Stream[Int]) => {
        println(l.headOption.get)
        Some(l.headOption.get, go(l.take(1)))
      })
    }
